...
describe User do
  ...
  it "should requires a valid or no color" do
    u = create_user(:color => "")
    u.errors.on(:color).should be_nil
    
    u = create_user(:color => "#dd")
    u.errors.on(:color).should_not be_nil
    
    u = create_user(:color => "#397bbf")
    u.errors.on(:color).should be_nil
  end
  ...
  it "should default the display name to the user name" do
    user = create_user
    user.display_name.should eql(user.login)
  end
